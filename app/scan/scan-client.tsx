...
function extractCodeFromUrl(text: string): string | null {
  ...
  //  - "J7YQR"
  //  - "https://yourdomain.com/redeem/J7YQR"
  //  - "https://yestodeals.com/redeem/J7YQR?x=1"
      ...
      <section className="space-y-3 rounded-3xl border border-slate-200 bg-white p-4 text-sm shadow-sm">
        ...
        {!error && (
          <p className="text-xs text-slate-500">
            {isScanning
              ...
              : detectedCode
              ? "Code detected. Use the button below to open the redemption page."
        ...
        {lastResult && (
          <div className="rounded-2xl bg-slate-50 px-3 py-2 text-[11px] text-slate-600">
            ...
            <p className="mt-1 break-words font-mono text-[11px]">{lastResult}</p>
          </div>
        )}
        ...
        {detectedCode && (
          <div className="rounded-2xl bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
            ...
            <p className="mt-1 font-mono text-sm font-semibold">{detectedCode}</p>
            <p className="mt-1 text-[11px]">
              Tap the button below to open the redemption page.
            </p>

            <a
              href={`/redeem/${encodeURIComponent(detectedCode)}`}
