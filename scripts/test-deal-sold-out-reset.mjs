/**
 * SOLD OUT TEST (with reset)
 *
 * - Deletes all redemptions for DEAL_ID (reset)
 * - Verifies availability shows left = maxRedemptions
 * - Creates COUNT QR codes
 * - Redeems them with concurrency + retry
 * - Expected: REDEEMED: 1, SOLD_OUT: COUNT-1
 *
 * NOTE: Local/dev only. Requires DATABASE_URL in env.
 */
...
async function main() {
  console.log("== SOLD OUT TEST (RESET) ==");
  ...
  console.log("\n0) Resetting deal redemptions in DB...");
  const del = await prisma.redemption.deleteMany({ where: { dealId: DEAL_ID } });
  console.log("✅ Deleted redemptions:", del.count);

  console.log("\n1) Checking availability...");
  const availRes = await fetch(`${BASE_URL}/api/deals/${DEAL_ID}/availability`);
  const avail = await availRes.json().catch(() => ({}));
  console.log("Availability:", avail);

  if (!avail?.ok) {
    console.error("❌ Availability endpoint failed.");
    process.exit(1);
  }

  console.log(`\n2) Creating ${COUNT} QR codes...`);
  const qrUrls = [];
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    if (!shortCode) {
      console.error("❌ Create response missing shortCode:", r);
      process.exit(1);
    }
  ...
  console.log(`\n3) Redeeming all QRs with concurrency=${CONCURRENCY}...`);
  ...
  if (redeemed === 1 && soldOut === COUNT - 1 && other === 0) {
    console.log("\n✅ PASS: 1 REDEEMED, rest SOLD_OUT.");
  } else {
    console.log("\n⚠️ Expected REDEEMED=1 and SOLD_OUT=COUNT-1 (with OTHER=0).");
    process.exit(1);
  }
}
